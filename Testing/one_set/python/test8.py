...
def test_remainder():
    ...
    a %= 3
    assert (
        verify.same(a.__str__(), line(), *stp)), "error in step 7: %= with interval with positive and negative borders"
    a.clear()
    a.add_interval(0, 3)
    a %= 3
    assert (
        verify.same(a.__str__(), line(), *stp)), "error in step 8: %= with interval with equal values on the balance"
    a.clear()
    a.add_interval(0, 6)
    a %= 3
    assert (
        verify.same(a.__str__(), line(), *stp)), "error in step 9: %= with interval with equal values on the balance"
    a.clear()
    a.add_point(1)
    a.add_interval(1, 3)
    a.add_point(3)
