...
def test_in_inverse_points_only():
    ...
    assert(verify.same(a.__str__(), line(), *stp) and 1 - a.points_only() and a.full()), "error in step 4: points_only on interval with -INF"
    a = a.inverse()
    assert(verify.same(a.__str__(), line(), *stp) and a.points_only() and 1 - a.full()), "error in step 5: inverse (-INF +INF)"
    a.add_interval(1, 10)
