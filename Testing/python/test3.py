...
def test_multi_add():
    ...
    a.add_interval(1, 10)
    a.add_point(10)
    assert verify.same(str(a), line(), *stp), "error in step 1: adding border point"

    b = a.add_interval(10, 11)
    assert verify.same(str(a), line(), *stp) and b, "error in step 2: meet 2 intervals: last - interval"

    a.add_interval(11, 20)
    b = a.add_point(11)
    assert verify.same(str(a), line(), *stp) and b, "error in step 3: meet 2 intervals: last - point"

    a.add_interval(20, 30)
    a.add_point(30)
    a.add_point(1)
    assert verify.same(str(a), line(), *stp), "error in step 4: random additions"
    ...
    a.add_point(31)
    a.add_point(32)
    assert verify.same(str(a), line(), *stp), "error in step 6: points in left and right"

    a.add_interval(-10, 1)
    assert verify.same(str(a), line(), *stp), "error in step 7: coating interval"
    ...
    a.add_point(-21)
    assert verify.same(str(a), line(), *stp), "error in step 8: points formatting"

    a.add_interval(a.minimal, -21)
    a.add_interval(35, a.maximal)
    assert verify.same(str(a), line(), *stp), "error in step 9: +-INF"
