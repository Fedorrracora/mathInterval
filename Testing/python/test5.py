...
def test_inverse():
    ...
    a.add_point(1)
    a.add_point(10)
    assert(verify.same(a.inverse().__str__(), line(), *stp)), "error in step 3: inverse one interval with points around"
    a.clear()
    ...
    assert(verify.same(a.inverse().__str__(), line(), *stp)), "error in step 5: inverse many random points and intervals"
    a.clear()
