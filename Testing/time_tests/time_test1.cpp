...
TEST(TIME_TEST, inverse) {
    ...
    double coef1, coef2, coef3, coef4, tm1, tm2;
    for (const int D : {1000, 10}) {
        const auto [a, tm] = verifier_tests::many_data(D);
        double t1, t2;
        ...
        const auto t = t2 / t1;
        if (D == 1000) coef1 = t1, coef2 = t2, tm1 = tm;
        else coef3 = t1, coef4 = t2, tm2 = tm;
    ...
    std::cout << "----- The following text is needed only for the parser -----\n";
    std::cout << "1000 10\n";
    std::cout <<  "inverse() " << fp(tm1) << ' ' << fp(coef1) << ' ' << fp(coef1 / ITERATIONS) << ' ' <<
        fp(coef3) << ' ' << fp(coef3 / ITERATIONS) << ' ' << fp(coef1 / coef3 / 100) << '\n';
    std::cout << "_inverse() " << fp(tm2) << ' ' << fp(coef2) << ' ' << fp(coef2 / ITERATIONS) << ' ' <<
