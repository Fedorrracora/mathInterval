...
| `add_point(x)`, `remove_point(x)`                                | `add_point(x)`, `remove_point(x)`                    | `O(log n)` | Добавляет или удаляет точку. Возвращают true, если множество изменилось. Стоит учесть, что добавляемая точка не может быть -INF и +INF                                                                        |
