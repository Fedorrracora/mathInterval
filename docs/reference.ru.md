...
| `print()`                                                        | `str()` (внешний built-in оператор)                  | `O(n)`     | Возвращает строковую запись текущего множества (математическая форма). В C++ хранимый тип должен иметь оператор \<\< для `std::ostream`. В Python тип должен иметь оператор `__str__`                         |
