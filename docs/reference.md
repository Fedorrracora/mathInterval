...
| `add_point(x)`, `remove_point(x)`                                  | `add_point(x)`, `remove_point(x)`                 | `O(log n)` | Add or remove a point. Return true if the set changes. Note that the added point cannot be -INF and +INF                                                                   |
