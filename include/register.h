...
namespace formatter {
    struct test_t {
        ...
        test_t(std::string name, std::function<void(const std::string &)> func) : name(std::move(name)), func(std::move(func)) {}
    };
    ...
    inline void setup(const std::string &s, bool &conf, bool &begin_reading) {
        ...
        if (verify::same(s, "----- The following text is needed only for the formatter -----", false, true, false)) {
            begin_reading = true;
        }
    }
}
...
namespace formatter { \
void REGISTER_CONCAT_(formatter_test_func_, priority) (const std::string& (input)); \
static bool REGISTER_CONCAT_(formatter_dummy_, priority) = []() { \
tests.emplace_back(#test_name, REGISTER_CONCAT_(formatter_test_func_, priority)); \
return true; \
}(); \
} \
void formatter::REGISTER_CONCAT_(formatter_test_func_, priority) (const std::string& (input))
#define REGISTER(test_name) REGISTER_IN(test_name, s, __COUNTER__) { static bool conf = false, begin_reading = false; formatter::setup(s, conf, begin_reading); }
